...
function cycle(model, num)
  ...
  local cyc = function (pathmode, model)
  ...
  if not p:hasSelection() then
    cyc(model.attributes["pathmode"], model)
  else
...
function cycleArrow(model, num)
  ...
  local cyc = function(farrow, rarrow, model)
    local states = {
      { false, false },
      { true, false },
      { false, true },
  ...
  if not p:hasSelection() then
    ...
    cyc(model.attributes["farrow"], model.attributes["rarrow"], model)
